...
- **移除未披露的第三方擦弹统计上报**：
  - 删除擦弹结算后向 `http://veyu.me/api/record` 上传用户 ID、投入金额、奖励倍率、奖励金额、盈亏和时间戳的逻辑。
...
- **修复旧插件卸载后安装新版插件的启动报错**：
  - 数据库迁移脚本不再硬编码 `data.plugins.astrbot_plugin_fishing` 旧插件包路径，改为从实际 `migrations_dir` 按文件路径加载。
  - 道具效果自动注册改为使用当前运行时包名，兼容 `astrbot_plugin_fishing_again` 等实际安装目录。
  - 新增动态插件路径回归测试，覆盖迁移加载和效果包路径配置。

## v2.6.0 (21点 + 拉杆机 + 骰宝增强 - 全新博弈系统)

- **🃏 新增21点(Blackjack)游戏系统**：
  - 完整的21点规则实现，支持系统庄家与玩家开庄两种模式
  - **加倍下注**：首2牌时可加倍（仅限2牌），获得1张额外牌后自动停牌
  - **分牌**：两张同点数可分为两手独立操作，各手独立结算
  - **保险**：庄家明牌为A时可购买保险（下注额50%），庄家Blackjack时赔付2:1
  - **智能超时**：操作超时自动模拟「12点以下抽牌，12点及以上停牌」策略
  - **图片模式**：默认图片渲染输出，深色主题牌桌，支持 `/21点模式` 管理员切换
  - **连胜/连败机制**：连胜3局额外奖励（默认10%），连败3局发放安慰金（默认500）
  - **WebUI可配置**：下注范围、开庄门槛、等待时间、超时时间、连胜/连败奖励均可配置
  - 新增命令：`/21点`、`/21点开庄`、`/21点加入`、`/抽牌`、`/停牌`、`/加倍`、`/分牌`、`/买保险`、`/21点状态`、`/21点开始`、`/21点帮助`、`/21点模式`

- **🎰 新增拉杆机游戏系统**：
  - 7个海洋主题符号（🐟🦀🐙🦈🐳💎🌟），加权随机采样（CSPRNG种子 + Mersenne Twister）
  - **4档位**：铜桌(1万)、银桌(10万)、金桌(100万)、至尊桌(500万)
  - **三同赔率**：×5 / ×8 / ×15 / ×30 / ×60 / ×120 / ×250
  - **两同赔率**：低级符号×1(返还本金)，鲸鱼×2，宝石×3，海星×5
  - **累积奖池(Jackpot)**：金桌/至尊桌可触发全额Jackpot或10% Mini Jackpot
  - **保底机制**：连续20次未中奖自动保底两同
  - **幸运时段**：每日随机2小时，高级符号概率提升50%
  - **连转模式**：支持一次最多10连转，自动统计汇总
  - **每日限额**：默认每日50次，跨日自动重置
  - **理论回报率约96%**：略有庄家优势，适合后期消耗
  - 新增命令：`/拉杆`、`/连转`、`/奖池`、`/拉杆记录`、`/拉杆帮助`、`/拉杆模式`

- **🎲 骰宝系统增强**：
  - **玩家开庄**：新增 `/玩家开庄` 命令，玩家可担任庄家（最低100万金币）
  - **骰宝记录**：新增 `/骰宝记录` 查看历史开奖记录
  - **消息模式配置**：骰宝配置项迁入WebUI，支持倒计时、下注范围、消息模式等配置

- **📊 统一读博记录系统**：
  - 全部使用「读博」替代「赌博」，规避QQ违规词检测
  - 新增 `/读博记录` 命令，统一展示21点、骰宝、拉杆机的博弈历史
  - 自动统计总局数、总盈亏、胜率、最大单笔盈亏

- **🔧 已有系统优化**：
  - 骰宝/21点/拉杆机默认图片模式输出，文本模式可选
  - 所有博弈游戏逾期借款检查统一化
  - _conf_schema.json 新增骰宝、21点、拉杆机三大配置区

- **🐛 Bug修复**：
  - 修复21点分牌(Split)后游戏卡死的问题：主手结束后分牌手的状态检查逻辑错误，导致无法继续操作

- **🔧 游戏模块开关**：
  - 新增 `/游戏开关` 管理员命令，支持per-group控制21点/骰宝/拉杆机的启用与禁用
  - 开关状态持久化存储，重启后自动恢复
  - 所有相关命令入口均添加开关检查，关闭后给出友好提示

- **✅ 测试覆盖**：
  - 新增 `tests/test_new_features.py`（21点+骰宝 5大测试组）
  - 新增 `tests/test_v1_3_1_changes.py`（术语/模式/智能超时/记录 11项）
  - 新增 `tests/test_slot.py`（拉杆机核心逻辑 20项）
  - 新增 `tests/test_slot_edge_cases.py`（边界/漏洞/性能 57项）
  - 新增 `tests/test_split_fix.py`（分牌修复 6项：is_active单元+集成）
  - 总计新增 99 项自测全部通过

## v2.5.0 (钓鱼权重算法重构 - 概率修复 + EV拟合 + 缓存优化)

- **🔴 修复核心概率缺陷**：
  - `coins_chance`（高价值加成）在原版算法中等比放大所有权重，归一化后实际概率完全不变。引入基于二分查找的期望价值（EV）拟合算法，确保装备加成倍率精确反映在收益数学期望上。
  - 修复鱼池基础权重分配逻辑：同星级内所有鱼的基础获取概率强制拉平（均设为 1.0），解决原版"越贵的鱼越容易出"的基础概率倒挂问题。

- **✨ 新增 `FishWeightService` 服务**：
  - 权重算法独立为 `FishWeightService`，在 `main.py` 中完成服务注册与依赖注入。
  - 引入天花板/地板保护逻辑，防止极端加成导致异常权重。
  - 支持负值 `coins_chance`，增加期望下界保护。

- **⚡ 性能优化**：
  - 新增 `OrderedDict` 内存缓存机制（LRU 淘汰），默认最大 1000 条目。
  - 缓存读写使用 `threading.Lock` 保护，确保自动钓鱼后台线程与前台指令并发安全。
  - 返回权重列表副本，防止外部修改污染缓存。

- **🔧 修复鱼饵续装逻辑**：
  - 修复限时鱼饵无法同类续装的问题。
  - 优化续装流程：优先续装同款鱼饵 → 同款耗尽后随机选择库存鱼饵兜底。
  - 续装时自动重置限时鱼饵的生效开始时间。

- **🔧 修复价值乘数获取**：
  - 修复鱼饵 `value_modifier` 和饰品 `bonus_coin_modifier` 的取值问题（乘数型默认值 1.0 转增量型 0.0）。

- **✅ 新增测试覆盖**：
  - 新增 `tests/test_weight.py`，73 项参数化矩阵测试（8 星级 × 9 种加成 + 边界用例），全部通过。
