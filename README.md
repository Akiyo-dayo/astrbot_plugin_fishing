...
- **🆕 21点(Blackjack)游戏**：加倍/分牌/保险/连胜连败/智能超时/图片模式，完整规则实现
- **🆕 拉杆机游戏**：7海洋主题符号，4档位，累积奖池/Jackpot/保底/幸运时段/连转模式
- **🆕 统一读博记录系统**：跨游戏博弈历史统计（21点/骰宝/拉杆机），规避QQ违规词
- **🆕 骰宝游戏全面重构**：完全重写骰宝游戏系统，图片化UI、多会话支持、独立服务层、倒计时系统、丰富的下注类型
- **🆕 骰宝玩家开庄**：玩家可担任庄家，新增骰宝记录查询
...
- 🃏 新增21点(Blackjack)游戏：加倍/分牌/保险/连胜连败/智能超时/图片模式
- 🎰 新增拉杆机游戏：7海洋符号×4档位，累积奖池/保底/幸运时段/连转
- 🎲 骰宝增强：玩家开庄 + 骰宝记录 + WebUI配置
- 📊 统一读博记录系统：跨游戏博弈历史统计，规避QQ违规词
- 🔧 新增 `/游戏开关` 管理员命令，per-group控制21点/骰宝/拉杆机的启用与禁用
- ✅ 新增99项自测全部通过
...
| `/金币` | - | 查看金币余额 |
| `/钓鱼银行` | - | 查看银行余额、免费提现额度、待确认预约和定期数量 |
| `/钓鱼银行 存款 [金额]` | `/钓鱼存款 [金额]` | 将钱包金币存入银行 |
| `/钓鱼银行 取款 [金额]` | `/钓鱼取款 [金额]` | 从银行取出金币 |
| `/钓鱼银行 预约取款 [金额]` | - | 创建大额取款预约 |
| `/钓鱼银行 确认预约` | - | 确认已到时间的大额取款预约 |
| `/钓鱼银行 取消预约` | - | 取消当前待确认的大额取款预约 |
| `/钓鱼银行 定期` | - | 查看定期存款规则和档位 |
| `/钓鱼银行 定期存款 [金额] [天数]` | - | 创建定期存款 |
| `/钓鱼银行 定期列表` | - | 查看我的定期存款 |
| `/钓鱼银行 定期取出 [编号]` | - | 领取已到期定期存款 |
| `/钓鱼银行 提前取出 [编号]` | - | 提前取出定期本金，不获得到期收益 |
...
- 单笔 5,000,000 金币及以上取款需要先预约，默认 24 小时后可确认取出
- 支持 1/3/7 天定期存款，默认收益率为 0.2%/0.8%/2.0%
- 定期本金从银行活期余额转入，锁定期间不能取款，到期后领取本金和收益
...
#### 🃏 21点(Blackjack)游戏（v2.6.0新增）

- **完整规则**：标准21点规则实现，支持系统庄家和玩家开庄
- **加倍下注**：初始2牌时可加倍，获得1张额外牌后自动停牌
- **分牌功能**：两张同点数可分为两手独立操作，各手独立结算
- **保险机制**：庄家明牌为A时可购买保险（下注额50%），庄家Blackjack时赔付2:1
- **智能超时**：操作超时自动模拟「12点以下抽牌，12点及以上停牌」策略
- **连胜/连败**：连胜3局额外奖励（默认10%），连败3局发放安慰金（默认500）
- **图片模式**：默认图片渲染输出，深色主题牌桌，支持管理员切换

#### 🎰 拉杆机游戏（v2.6.0新增）

- **海洋主题**：7个海洋主题符号（🐟🦀🐙🦈🐳💎🌟），从常见到稀有
- **4个档位**：铜桌(1万)、银桌(10万)、金桌(100万)、至尊桌(500万)
- **三同赔率**：×5 / ×8 / ×15 / ×30 / ×60 / ×120 / ×250
- **累积奖池**：金桌/至尊桌可触发Jackpot（独得全部奖池）或Mini Jackpot（10%奖池）
- **保底机制**：连续20次未中奖自动保底两同（可配置）
- **幸运时段**：每日随机2小时，高级符号概率提升50%
- **连转模式**：支持一次最多10连转，自动汇总统计
- **理论回报率**：约96%，略有庄家优势，适合后期消耗
...
| `/继续` | - | 继续命运之轮游戏 |
| `/放弃` | - | 放弃命运之轮游戏 |
| `/21点 [金额]` | `/二十一点`、`/blackjack` | 开始21点游戏（系统庄家） |
| `/21点开庄` | `/21点当庄` | 玩家开庄21点 |
| `/21点加入 [金额]` | `/加入21点` | 加入玩家开的21点 |
| `/抽牌` | `/要牌`、`/拿牌` | 21点要牌 |
| `/停牌` | `/不要了`、`/不要牌` | 21点停牌 |
| `/加倍` | `/21点加倍`、`/doubledown` | 加倍下注（仅限初始2牌时） |
| `/分牌` | `/21点分牌`、`/split` | 分牌（两张同点数时） |
| `/买保险` | `/21点保险`、`/insurance` | 庄家A时买保险 |
| `/21点状态` | `/21点查看` | 查看当前21点状态 |
| `/21点开始` | `/21点强制开始` | 强制开始发牌 |
| `/21点帮助` | `/21点规则`、`/21点说明` | 查看21点帮助 |
| `/21点模式` | - | [管理员] 切换图片/文本模式 |
| `/读博记录` | `/读博历史`、`/gambling_records` | 查看博弈统计记录 |
| `/骰宝记录` | `/骰宝开奖记录` | 查看骰宝历史开奖 |
| `/玩家开庄` | `/我来当庄`、`/我当庄` | 骰宝玩家开庄 |
| `/拉杆 [档位]` | `/拉杆机`、`/slot` | 拉杆机（默认铜桌，可选铜/银/金/至尊） |
| `/连转 [档位] [次数]` | `/拉杆连转`、`/multi_spin` | 连续拉杆（最多10次） |
| `/奖池` | `/拉杆奖池`、`/jackpot` | 查看拉杆机累积奖池 |
| `/拉杆记录` | `/拉杆历史` | 查看拉杆历史记录 |
| `/拉杆帮助` | `/拉杆说明` | 查看拉杆机帮助 |
| `/拉杆模式` | - | [管理员] 切换图片/文本模式 |
